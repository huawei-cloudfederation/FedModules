...
if [ "$MESOS_HOME" == "" ]; then
  ...
  exit 1
elif [ ! -d "$MESOS_HOME" ]; then
  echo -e "\nThe mesos Home Location '$MESOS_HOME' is Not VALID\n"
  exit 1
fi
...
HEADER_FILES=" 	-I $MESOS_HOME_DIR/include 
	       	-I $MESOS_HOME_DIR/src 
	       	-I $MESOS_HOME_DIR/build/include 
               	-I $MESOS_HOME_DIR/build/src 
               	-I $UB_LIBPROCESS_PATH/include  
	       	-I $UB_LIBPROCESS_PATH/3rdparty/stout/include 
		-I $UB_LIBPROCESS_PATH/3rdparty/stout/include 
		-I $B_THIRD_PARTY/picojson-1.3.0 
